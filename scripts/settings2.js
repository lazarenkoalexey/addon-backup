...
var listBackups = {};
var backupTemplate = "b8ed09fd-4f6f-4343-b6e3-1c06a849c139";
var nodesArray = [];
var nodesName = {};
var ids = [];
var conteneur = '';
    ...
    if (envInfo.env.status == "1") {
        ...
        for (var j = 0; node = envInfo.nodes[j]; j++) {
            for (var m = 0; add = node.addons[m]; m++) {
                ...
                jelastic.marketplace.console.WriteLog("backupTemplate- >" + backupTemplate);
    ...
    if (FileReadResponse.result != 0) {
        ...
        if (plan.last_update > local_date || plan.user != account) {
            ...
            plan.backup_plan.forEach(function(objectBackup) {
                ...
                var toDisplay = objectBackup["date"].replace('T', ' ') + " " + objectBackup["path"] + " " + objectBackup["size"] + " " + plan.user;
